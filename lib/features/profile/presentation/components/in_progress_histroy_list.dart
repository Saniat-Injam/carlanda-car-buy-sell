...
class InProgressHistroyList extends StatelessWidget {
  InProgressHistroyList({super.key});
  final MyHistoryController controller = Get.find<MyHistoryController>();
  @override
  Widget build(BuildContext context) {
    return ListView.separated(
      itemBuilder: (context, index) =>
          CommonHistoryCard(history: controller.inProgressHistorys[index]),
      separatorBuilder: (context, index) => 16.heightSpace(),
      itemCount: controller.inProgressHistorys.length,
    );
  }
}
